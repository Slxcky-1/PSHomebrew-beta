#!/bin/bash
# PSHomebrew Discord Bot - Ubuntu GUI Installation Script
# Run with: bash install-ubuntu-gui.sh
...
# Welcome screen
$DIALOG --title "PSHomebrew Discord Bot" --msgbox "Welcome to the PSHomebrew Discord Bot installer for Ubuntu/Debian!\n\nThis wizard will guide you through the installation process." 10 60
...
if $DIALOG --title "System Update" --yesno "Update system packages before installation?" 7 50; then
    (sudo apt update && sudo apt upgrade -y) 2>&1 | $DIALOG --title "Updating System" --programbox 20 70
fi
...
if command -v node &> /dev/null; then
    ...
    if [ "$NODE_VERSION" -ge 18 ]; then
        $DIALOG --title "Node.js Found" --msgbox "Node.js $(node --version) is already installed!" 7 50
    else
        $DIALOG --title "Updating Node.js" --infobox "Updating Node.js to latest LTS..." 5 50
        (curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt install -y nodejs) 2>&1 | $DIALOG --title "Installing Node.js" --programbox 20 70
    fi
else
    $DIALOG --title "Installing Node.js" --infobox "Installing Node.js LTS..." 5 50
    (curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt install -y nodejs) 2>&1 | $DIALOG --title "Installing Node.js" --programbox 20 70
fi

# Install build essentials
$DIALOG --title "Installing Tools" --infobox "Installing build essentials..." 5 50
sudo apt install -y build-essential 2>&1 | $DIALOG --title "Installing Build Tools" --programbox 20 70
